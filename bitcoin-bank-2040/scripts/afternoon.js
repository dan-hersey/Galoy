...
function renderCreditOverview(state) {
  ...
  // Collateral value
  const collateralValueEl = document.getElementById('collateral-value');
  if (collateralValueEl) collateralValueEl.textContent = formatUSD(balances.vault.usd, false);

  const collateralBtcEl = document.getElementById('collateral-btc');
  if (collateralBtcEl) collateralBtcEl.textContent = formatBTC(balances.vault.btc);
}
