...
function renderFamilyAccounts(state) {
  ...
  if (sofia) {
    const sofiaSats = document.getElementById('sofia-sats');
    const sofiaUsd = document.getElementById('sofia-usd');
    if (sofiaSats) sofiaSats.textContent = formatSats(sofia.balance.btc);
    if (sofiaUsd) sofiaUsd.textContent = formatUSD(sofia.balance.usd, false);
  }

  if (marco) {
    const marcoSats = document.getElementById('marco-sats');
    const marcoUsd = document.getElementById('marco-usd');
    if (marcoSats) marcoSats.textContent = formatSats(marco.balance.btc);
